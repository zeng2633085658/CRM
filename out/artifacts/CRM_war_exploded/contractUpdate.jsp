...
<blockquote class="layui-elem-quote"><h2>修改合同资料</h2></blockquote>

<form class="layui-form" action="<%=basePath%>contractServlet?action=updateSave" method="post" name="form1">
        ...
        <div class="layui-inline">
            ...
            <div class="layui-inline">
                <input type="text"  name="title" lay-verify="title" autocomplete="off"
                       placeholder="请输入合同编号" class="layui-input" value="${contract.contract_no}">
            </div>
        </div>
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text"  name="contract_name" lay-verify="title1" autocomplete="off"
                       placeholder="请输入合同名称" class="layui-input" value="${contract.contract_name}">
            </div>
        </div>
    </div>
    <div class="layui-form-item">
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" name="contract_time" id="joindate" lay-verify="date"
                       placeholder="yyyy-MM-dd" lay-verify="required" autocomplete="off" class="layui-input" value="${contract.contract_time}">
            </div>
        </div>
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" name="due_time" id="workdate"
                       placeholder="yyyy-MM-dd" lay-verify="required" autocomplete="off" class="layui-input" value="${contract.due_time}">
            </div>
        </div>
    </div>
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" name="total_money" lay-verify="required" autocomplete="off"
                       placeholder="请输入合同金额" class="layui-input" value="${contract.total_money}">
            </div>
        </div>
    </div>
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-block">
                <select name="pay_type" lay-filter="aihao">
                    <c:if test="${contract.pay_type=='按月支付'}">
                    ...
                    <c:if test="${contract.pay_type=='按季度支付'}">
                        <option value="按月支付">按月支付</option>
                        <option value="按季度支付" selected="">按季度支付</option>
                        <option value="按半年支付">按半年支付</option>
                        <option value="按年支付">按年支付</option>
                    </c:if>
                    <c:if test="${contract.pay_type=='按半年支付'}">
                        <option value="按月支付">按月支付</option>
                        <option value="按季度支付">按季度支付</option>
                        <option value="按半年支付" selected="">按半年支付</option>
                        <option value="按年支付">按年支付</option>
                    </c:if>
                    <c:if test="${contract.pay_type=='按年支付'}">
                        <option value="按月支付">按月支付</option>
                        <option value="按季度支付">按季度支付</option>
                        <option value="按半年支付">按半年支付</option>
                        <option value="按年支付" selected="">按年支付</option>
                    </c:if>
                </select>
            </div>
        </div>
        <div class="layui-inline">
            ...
            <div class="layui-input-block">
                <select name="status" lay-filter="aihao">
                    <c:if test="${contract.status=='已付款'}">
                        <option value="已付款" selected="">已付款</option>
                        <option value="已签订">已签订</option>
                        <option value="服务中">服务中</option>
                        <option value="已完成">已完成</option>
                    </c:if>
                    <c:if test="${contract.status=='已签订'}">
                        <option value="已付款">已付款</option>
                        <option value="已签订" selected="">已签订</option>
                        <option value="服务中">服务中</option>
                        <option value="已完成">已完成</option>
                    </c:if>
                    <c:if test="${contract.status=='服务中'}">
                        <option value="已付款">已付款</option>
                        <option value="已签订">已签订</option>
                        <option value="服务中" selected="">服务中</option>
                        <option value="已完成">已完成</option>
                    </c:if>
                    <c:if test="${contract.status=='已完成'}">
                        <option value="已付款">已付款</option>
                        <option value="已签订">已签订</option>
                        <option value="服务中">服务中</option>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block">
                <textarea placeholder="请输入内容" class="layui-textarea" name="remark">${contract.remark}</textarea>
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
