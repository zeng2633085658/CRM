...
//	}
	...
	//导出用户信息
	public void addUsers(String docsPath){
		ExportExcel<UsersVo> ex = new ExportExcel<UsersVo>();
		String[] headers = { "ID", "用户名称", "密码", "岗位", "手机" ,"身份证"
		,"入职时间","工资","账户类型","状态(可编辑)"};
		List<UsersVo> dataset =new BaseDAO().findPageUsers(0,1000);
		try{
			OutputStream out = new FileOutputStream(docsPath + FILE_SEPARATOR
					+ "export2003_a.xls");
			ex.exportExcel(headers, dataset, out);
